...
public class BoardManager : NetworkBehaviour
{
	...
	private void Awake()
	{
		// Ensure only one instance of BoardManager exists
	...
	{
		TryDestroyVisualPieceServerRpc(SquareToString(position));
	}

	[ServerRpc(RequireOwnership = false)]
	public void TryDestroyVisualPieceServerRpc(string positionName)
	{
		// Convert the position name back to a Square object.
		Square position = new Square(positionName);
		...
		if (visualPiece != null)
		{
			...
			// Notify all clients to destroy the piece.
			TryDestroyVisualPieceClientRpc(positionName);
		}
	}

	[ClientRpc]
	private void TryDestroyVisualPieceClientRpc(string positionName)
	{
		// Convert the position name back to a Square object.
		Square position = new Square(positionName);
